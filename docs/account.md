...
| `account-core` | ✅ 운영 | Entity 11개, Repository, Multi-tenant 격리 본체(`HouseholdContext` + Hibernate `@Filter`) + Flyway V1~V6 |
...
GET    /web/report                       # 기간/연 결산 — from~to + 프리셋(이번 달/지난 달/올해/작년), 수입·지출·잉여·카테고리별 (WebReportController)
...
- 가구 초대 플로우 (이메일)
- OWNER/MEMBER 역할 분리 강화 (예산 수정은 OWNER만 등) — 관리자 페이지의 OWNER 게이트는 v1.5 항목 중 일부를 당겨 적용한 상태
- 회원가입 화면
- 탈퇴/데이터 삭제 자동 배치
