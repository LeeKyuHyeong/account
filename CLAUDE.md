...
- **영수증 인제스천**: `WebReceiptController` → `ReceiptIngestionService.ingest` (@Transactional 단일 트랜잭션) → `ReceiptStorage.store` (디스크 — 원본 무변형) + `Receipt` insert + `MerchantHistoryProvider.getRecentHistory` + `ReceiptAnalysisService.analyze` (다운스케일 → Claude, temperature 0 + 재시도 + max_tokens 잘림 감지) + 카테고리 fallback 매칭 + DRAFT `Transaction` insert → `receipts/confirm.html` 렌더 (전체필드 편집 + DRAFT→CONFIRMED 확정은 `POST /web/transactions/{id}` 재사용).
- **영수증 분석 정확도 이력**: `GET /web/receipts/analysis` (더보기 진입) → `ReceiptAccuracyService.listRecent(30)` — `ocr_raw_json`(AI 원본) vs 거래 저장값 필드별 비교 + `summarize()` (필드별 수정 횟수/7일 추이, on-the-fly — 배치 없음). 사용자가 이 화면 보고 프롬프트를 보강하는 루프가 운영 방식.
...
- **가구 설정 (OWNER 전용)**: `WebAdminController` (`/web/admin`) → `AdminUserService.listMembers` (멤버 목록) + `InviteCodeService.generate/listActive/revoke` (초대코드 발급/조회/취소 — 혼동문자 제외 8자, 충돌 시 재시도, 발급 3일 뒤 만료, 목록은 취소·만료 제외, 취소는 `findByIdAndHouseholdId` 가드). 가구 경계는 `findByHouseholdId` 로 직접 가드 (User/HouseholdMember/InviteCode 비격리). (비번 재설정은 카카오 전환으로 제거됨.)
...
- 전체 테스트: `./gradlew test` — 2026-09-21 기준 테스트 클래스 20개·91건(api 71·core 8·ai 12), 전부 단위 테스트 — Mockito(`@ExtendWith(MockitoExtension.class)`) 13개, 순수 단위·MockMvc standalone(`SecurityConfigEntryPointTest`) 7개. DB·Docker 불필요. **통합 테스트 계층은 아직 없다**(Testcontainers 의존성만 선언, `@SpringBootTest`·`@DataJpaTest` 0건)
