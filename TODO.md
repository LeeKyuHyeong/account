...
```
M0 기반:    ▓▓▓▓▓▓░  레이아웃 + app.css + 에러페이지 완료 / WebPingController 제거는 M1
M1 핵심:    ░░░░░░░  홈 + 거래 목록/입력/수정
M2 영수증:  ░░░░░░░  업로드 → 분석 → 컨펌
...
- [ ] **홈** (`GET /web/home`) — 이번 달 수입 / 지출 / 잉여 카드 + 예산 초과 경고 banner + 화면 이동 내비
- [ ] **거래 목록** (`GET /web/transactions`) — 날짜별 그룹, 필터(from / to / categoryId / type / status), **페이지네이션**(무한스크롤 대체), DRAFT 뱃지
- [ ] **거래 입력** (`GET/POST /web/transactions/new`) — amount / category / occurredAt / merchant / paymentMethod / memo, 서버 검증 에러 필드별 표시
- [ ] **거래 수정** (`GET/POST /web/transactions/{id}`) — category / status PATCH (DRAFT→CONFIRMED 일방향). `transaction_history` 적재는 기존 서비스가 처리
- [ ] 통화 / 날짜 포맷 — 서버 측 `#numbers` / `#temporals` (Flutter intl 대체)
- 검증: 수동 거래 입력 → 목록 반영 → 수정 → 상태 변경. 다른 가구 거래 0건(격리).
